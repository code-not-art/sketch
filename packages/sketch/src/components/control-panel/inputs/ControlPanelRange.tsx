...
export const ControlPanelRange = (props: {
	...
	const clampStart = (value: number) => (value > parameter.startMax ? parameter.startMax : value);
	const clampEnd = (value: number) => (value < parameter.endMin ? parameter.endMin : value);
		...
		<>
			<div className="card flex align-items-center justify-content-center mb-2 ">
				<div className="w-full">
					...
					<br />
					<StyledInput value={internalValue[0]} onChange={(event) => event.value && onStartChange(event.value)} />
					<StyledInput value={internalValue[1]} onChange={(event) => event.value && onEndChange(event.value)} />
					<Slider
						range
						className="w-full"
						value={internalValue}
