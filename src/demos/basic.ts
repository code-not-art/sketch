import { Color, Utils, Vec2, grid, type FillSelection, type Stroke } from '@code-not-art/core';
import {
	ControlPanel,
	MultiSelectUtils,
	Parameters,
...
const controls = {
	...
	colors: ControlPanel('Colors', {
		...
		hueRange: Parameters.range({
			label: 'Hue Variation Range',
...
const draw: SketchDraw<CustomControls, CustomData> = ({ canvas, palette, params, rng }, _data) => {
	...
	const hueClamp = (input: number) => clamp(input < 1 ? 360 + input : input, { min: 1, max: 360 });
	...
	const shapeColors = [
		...array(8).map(
			(index) =>
				new Color({
					h: hueClamp(((rng.float(hueRange[0], hueRange[1]) * 360 + baseHue) % 359) + 1),
					s: saturation,
					v: 100,
				}),
		),
	...
	const drawMarker = (config: { location: Vec2; scale?: number }) => {
		...
		stroke = {
			color: palette.rng.chooseOne(shapeColors),
			width: strokeWidth,
		};
		palette.rng.push('fill');
		if (rng.next() < fillChance) {
			fill = palette.rng.chooseOne(shapeColors);
		}
		palette.rng.pop();
