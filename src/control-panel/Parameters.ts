...
import type { ControlPanelConfig, ControlPanelElements, ControlPanelParameterValues } from './types/controlPanel.js';
import type {
...
const booleanParameter = (
	config: Identity<
		Partial<ControlPanelParameterBooleanConfig> & ControlPanelParameterBaseConfig<ControlPanelValueBoolean>
	>,
): ControlPanelParameterBoolean => {
	return {
		dataType: 'boolean',
...
const numberParameter = (
	config: Identity<
		Partial<ControlPanelParameterNumberConfig> & ControlPanelParameterBaseConfig<ControlPanelValueNumber>
	>,
): ControlPanelParameterNumber => {
...
const randomSeedParameter = (
	config: Identity<
		Partial<ControlPanelParameterRandomSeedConfig> & ControlPanelParameterBaseConfig<ControlPanelValueRandomSeed>
	>,
): ControlPanelParameterRandomSeed => {
	return {
		dataType: 'randomSeed',
...
const rangeParameter = (
	config: Identity<Partial<ControlPanelParameterRangeConfig> & ControlPanelParameterBaseConfig<ControlPanelValueRange>>,
): ControlPanelParameterRange => {
	const min = withDefault(config.min, 0);
	const max = withDefault(config.max, min >= 1 ? min * 10 : 1);
	const delta = max - min;
...
const stringParameter = (
	config: Identity<
		Partial<ControlPanelParameterStringConfig> & ControlPanelParameterBaseConfig<ControlPanelValueString>
	>,
): ControlPanelParameterString => {
	return {
		dataType: 'string',
...
} satisfies Record<ControlPanelParameterType, (config: any) => ControlPanelParameter>;
...
export const initialParameterValueMultiSelect = (
	...
	const defaultValue = config.options.reduce<Partial<Defined<ControlPanelParameterMultiSelect['initialValue']>>>(
		(acc, option) => {
			acc[option] = false;
			return acc;
		},
...
const isElementAParameter = (input: ControlPanelConfig<any> | ControlPanelParameter) => input && 'dataType' in input;

export const initialControlPanelValues = <TConfig extends ControlPanelConfig<ControlPanelElements>>(
	config: TConfig,
	...
	if (!config?.elements) {
		console.log(`Can't parse control panel section: ${JSON.stringify(config, null, 2)}`);
